...
export const updateProduct=createAsyncThunk('admin/updateProduct',async({id,productData},{rejectWithValue})=>{
     ...
     try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error('Server error:', errorData);
        throw new Error(errorData.message || 'Product updation failed');
      }
      const data = await response.json();
...
//Update User role
export const updateUserRole=createAsyncThunk('admin/updateUserRole',async({_id,role},{rejectWithValue})=>{
    try{
      console.log("userig ,rptt--" ,_id,role);
      
        const {data}=await axios.put(`/api/v1/admin/user/${_id}`,{role})
        return data;
